test_that("validations pass", {
  ...
  expect_message(validate_deck(test_path("testdata", "testdeck3.csv"), package = FALSE), "so using filename for title")
  suppressMessages(expect_message(validate_deck(test_path("testdata", "testdeck3.csv"), package = FALSE)))
  testdeck4 <- testdeck[, 1:2]
  write.csv(testdeck4, test_path("testdata", "testdeck4.csv"))
  expect_message(validate_deck(test_path("testdata", "testdeck4.csv"), package = FALSE), "so using filename for title")
  suppressMessages(expect_error(validate_deck(test_path("testdata", "testdeck4.csv"), package = TRUE)))
